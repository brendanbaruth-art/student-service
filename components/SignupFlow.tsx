...
export function SignupFlow() {
  ...
  return (
    <form className="rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_18px_35px_rgba(21,34,56,0.06)] sm:p-6">
      <div className="grid grid-cols-4 gap-2" aria-label="Signup progress">
        {steps.map((step, index) => (
          <div key={step} className="min-w-0">
            <div className={`h-2 rounded-full ${index === 0 ? "bg-[#5B7CFA]" : "bg-[#E5E7EB]"}`} />
            <p className="mt-2 truncate text-xs font800 text-[#667085]">{step}</p>
          </div>
        ))}
      </div>

      <section className="mt-8">
        <h2 className="text-xl font900 text-[#152238]">Step 1: How would you like to use Etudo?</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {["Find help", "Offer services", "Both"].map((option) => (
            <label key={option} className="flex min-h-12 items-center gap-3 rounded-md border border-[#D0D5DD] px-3 text-sm font800 text-[#172033]">
              <input name="role" type="radio" className="size-4 accent-[#5B7CFA]" required />
              {option}
            </label>
          ))}
        </div>
      ...
      <section className="mt-8">
        <h2 className="text-xl font900 text-[#152238]">Step 2: Basic information</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <label className="grid gap-2 text-sm font800 text-[#172033] sm:col-span-2" htmlFor="password">
            Password
            <span className="flex rounded-md border border-[#D0D5DD] bg-white focus-within:border-[#5B7CFA] focus-within:ring-4 focus-within:ring-[#5B7CFA]/10">
              <input
                id="password"
              ...
              <button
                type="button"
                className="grid min-h-11 w-12 place-items-center text-[#667085]"
            ...
            <span className="text-xs font600 text-[#667085]">Use at least 8 characters.</span>
          </label>
        </div>
      </section>

      <section className="mt-8">
        <h2 className="text-xl font900 text-[#152238]">Step 3: Student verification</h2>
        <p className="mt-2 text-sm leading-6 text-[#667085]">
          Students verify university affiliation before offering services and before accessing some booking features.
        </p>
        <div className="mt-4 rounded-md border border-[#B7E4D3] bg-[#E8F5EF] p-4 text-sm font700 text-[#26755B]">
          University email and student document checks keep the network safer for everyone.
        </div>
      </section>

      <section className="mt-8">
        <h2 className="text-xl font900 text-[#152238]">Step 4: Complete profile</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <label className="mt-6 flex items-start gap-3 text-sm font700 text-[#667085]">
        <input type="checkbox" className="mt-1 size-4 accent-[#5B7CFA]" required />
      ...
      <p className="mt-3 text-sm font700 text-[#B42318]">
