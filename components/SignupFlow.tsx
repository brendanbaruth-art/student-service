    ...
    <form className="rounded-lg border border-[var(--color-border)] bg-white p-5 shadow-[0_18px_35px_rgba(21,34,56,0.06)] sm:p-6">
      <div className="grid grid-cols-4 gap-2" aria-label="Signup progress">
        {steps.map((item, index) => (
          <div key={item} className="min-w-0">
            <div className={`h-2 rounded-full ${index <= step ? "bg-[var(--color-brand)]" : "bg-[var(--color-border)]"}`} />
            <p className="mt-2 truncate text-xs font800 text-[var(--color-text-secondary)]">{item}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
        ...
        <Button type="button" onClick={continueFlow}>
          {step === steps.length - 1 ? "Finish signup" : "Continue"}
        </Button>
      </div>
      {submitted ? (
        <p className="mt-3 rounded-md bg-[var(--color-success-soft)] px-3 py-2 text-sm font800 text-[var(--color-success)]">
          Your Etudo profile is ready to review.
        </p>
      ) : null}
    </form>
  );
