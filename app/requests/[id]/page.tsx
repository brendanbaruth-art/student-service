  ...
  return (
    <PageShell>
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
        <div className="rounded-lg border border-[#E5E7EB] bg-white p-6 shadow-[0_18px_35px_rgba(21,34,56,0.06)]">
          <span className="inline-flex rounded-full bg-[#EEF2FF] px-3 py-1 text-xs font900 text-[#5B7CFA]">
            {request.category}
          </span>
          <h1 className="mt-4 text-4xl font900 tracking-tight text-[#152238]">
            {request.title}
          </h1>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={text as string} className="flex items-center gap-3 rounded-md bg-[#F8F7F3] p-4 text-sm font900 text-[#172033]">
                <Icon size={17} className="text-[#5B7CFA]" aria-hidden />
                {text as string}
              </div>
            ))}
          </div>
          <section className="mt-8 rounded-lg bg-[#F8F7F3] p-5">
            <h2 className="text-xl font900 text-[#152238]">Task details</h2>
        ...
        <aside className="h-fit rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_18px_35px_rgba(21,34,56,0.06)] lg:sticky lg:top-24">
          <p className="text-sm font900 uppercase tracking-[0.14em] text-[#667085]">Budget</p>
          <p className="mt-2 text-3xl font900 text-[#152238]">{request.budget}</p>
          <div className="mt-5 grid gap-3 text-sm font800 text-[#667085]">
            <p>Posted by {request.postedBy} · {request.posterRating.toFixed(1)} rating</p>
            <p>{request.postedAgo}</p>
            <p>{request.mode}</p>
          </div>
          <div className="mt-6 rounded-md bg-[#E8F5EF] p-4 text-sm leading-6 text-[#26755B]">
          ...
          {matches.length ? (
            <div className="mt-6">
              <p className="text-sm font900 text-[#152238]">Suggested helpers</p>
